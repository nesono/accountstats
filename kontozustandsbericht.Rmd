...
```{r Monatsaufstellung, results='asis', eval=TRUE, echo=FALSE, fig.align='center', fig.cap=''}
monat <- c('Januar','Februar','März','April','Mai','Juni','Juli',
           'August','September','Oktober','November','Dezember')
for( year in rev(names(ydat))) {
  cat( paste('### Aufstellung für', year, '\n\n' ) )
  
  tydat <- ydat[[year]]
  mdat <- split( tydat, as.factor(tydat$Buchungstag$mon+1) )
  
  for( month in rev(names(mdat))) {
    cat( paste('#### Monat', monat[as.numeric(month)], year, '\n\n' ) )
    cat( 'In der Aufstellung werden alle Ein- und Ausgaben des Wertes nach sortiert.\n' )
    cat( 'Wobei Ausgaben rot und Einnahmen schwarz gekennzeichnet werden\n\n' )
    ...
    cat('\n\nIm folgenden wird die Einzelbilanz des Monats dargestellt.\n\n')
